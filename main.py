...
from imagegen.generate_image import main_generate_images
...
def generate_yes_no_questions(text):
    try:
        prompt = f"""
            Based on the following study material and the general knowledge any normal human would have on the given topic, generate:
...
def index():
    if request.method == 'POST':
        try:
            ...
            required_fields = ['topic', 'duration', 'key_points']
            ...
            print(topic,duration,key_points)
            # Generate script
            
            generator = VideoScriptGenerator(api_key=openai_api_key)
            ...
            main_generate_images(script_path, images_path)
...
def serve_video(video_name):
    return send_from_directory("./static/videos", video_name)
    ...
    if os.path.exists(VIDEO_DIR):
        ...
        for filename in os.listdir(VIDEO_DIR):
            ...
            if filename.lower().endswith(('.mp4', '.webm', '.ogg', '.mov', '.avi')):
                videos.append({
                    'name': filename,
                    'path': os.path.join(VIDEO_DIR, filename).replace('\\', '/')
                })
