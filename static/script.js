...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/api/videos')
        ...
        .then(videos => {
            if (videos.length === 0) {
                historyList.innerHTML = '<p>No videos found in the directory.</p>';
                return;
            }
            ...
            videos.forEach(video => {
                ...
                videoItem.className = 'video-item p-4 bg-gray-100 rounded-lg';
                
                const videoTitle = document.createElement('h3');
                videoTitle.className = 'text-lg font-semibold mb-2';
                videoTitle.textContent = video.name;
                
                const videoPlayer = document.createElement('video');
                videoPlayer.className = 'w-full rounded';
        ...
        .catch(error => {
            ...
            historyList.innerHTML = '<p class="text-red-500">Error loading videos. Please try again.</p>';
            historyList.classList.remove('hidden');
        });
});
