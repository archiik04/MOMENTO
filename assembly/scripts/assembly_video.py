...
def create_complete_srt(script_folder :str, 
    ...
    """
    script_folder=r"resources/scripts/script.json";
...
def get_files(folder, extensions):
    ...
    if os.path.isdir(folder):
        ...
        # Sort files numerically based on segment number
...
def create_srt(text: str, audio_file: AudioFileClip, outfile_name: str, duration: int, chunk_size=5):
    """
    Create SRT subtitle file from text and audio duration.
    """
    words = text.split()
    chars = " ".join(words)
    chars_count = len(chars)
    char_duration = audio_file.duration / chars_count if chars_count > 0 else 0
    ...
    srt_dir = "samples/subtitles/srt"
    os.makedirs(srt_dir, exist_ok=True)
    
    out = os.path.join(srt_dir, f"{outfile_name}.srt")
...
def create_intro_clip(background_image_path, duration, topic, font_path):
    """
    Create an intro video clip with a background image and centered text.
    """
    try:
        ...
        background = ImageClip(background_image_path).with_duration(duration)
...
def create_placeholder_image(width=1920, height=1080, text="No Image", font_path=None, font_size=50, text_color="white", bg_color="black"):
    ...
    try:
        ...
        # Save placeholder
        placeholder_path = "resources/placeholder.png"
        ...
        # Return a simple black image path
...
def create_video(image_folder: str, 
    ...
    # Create different clips with audio - KEEP IT SIMPLE like your working code
    for img, audio in zip(images, audio_files):
        audio_clip = AudioFileClip(audio)
        # This is the key line that works - simple audio attachment
        image_clip = ImageClip(img).with_duration(audio_clip.duration).with_audio(audio_clip)
        audio_durations.append(audio_clip.duration)
        print(f"Video Clip no. {images.index(img) + 1} successfully created")
        Start_duration += audio_clip.duration
    ...
    # Use the exact same write_videofile parameters as your working code
    final_video.write_videofile(output_file, fps=24, threads=os.cpu_count())
    print(f"Video created successfully: {output_file}")

def create_complete_srt(script_folder: str, audio_file_folder: str, outfile_path: str, chunk_size=10):
    """
    ...
    try:
        ...
        audio_clips = [AudioFileClip(x) for x in audio_files]
        subs = pysrt.SubRipFile()
        
        start_time = 5  # Start after intro
        n = 1
        ...
        for text, audio_clip in zip(script, audio_clips):
            ...
            if len(words) > chunk_size:
                for i in range(0, len(words), chunk_size):
                    ...
                    subtitle = pysrt.SubRipItem(
                        index=n,
                        start=pysrt.SubRipTime(seconds=int(start_time)),  # Convert to int
                        end=pysrt.SubRipTime(seconds=int(end_time)),      # Convert to int
                        text=chunk
                    )
                    subs.append(subtitle)
                    start_time = end_time
        ...
        print(f"✅ SRT file saved successfully at {outfile_path}")
...
if __name__ == "__main__":
    ...
    create_video(images_path, audio_path, script_path, font_path, "output/newtons_1st_law.mp4", with_subtitles=True)